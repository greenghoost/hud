...
local function loadSettings(settings)
    ...
    exports.ggcore:Notify(Lang:t("notify.hud_settings_loaded"), 'success')
    Wait(1000)
...
local function restartHud()
    ...
    exports.ggcore:Notify(Lang:t("notify.hud_restart"), 'error')
    ...
    Wait(2600)
    exports.ggcore:Notify(Lang:t("notify.hud_start"), 'success')
end
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            exports.ggcore:Notify(Lang:t("notify.load_square_map"), 'inform')
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            exports.ggcore:Notify(Lang:t("notify.loaded_square_map"), 'success')
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            exports.ggcore:Notify(Lang:t("notify.load_circle_map"), 'inform')
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            exports.ggcore:Notify(Lang:t("notify.loaded_circle_map"), 'success')
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function(_, cb)
    ...
    if Menu.isCineamticModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            exports.ggcore:Notify(Lang:t("notify.cinematic_off"), 'error')
        end
        DisplayRadar(true)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            exports.ggcore:Notify(Lang:t("notify.cinematic_on"), 'success')
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
...
CreateThread(function()
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if not (cache.vehicle and not IsThisModelABicycle(vehicle)) then
            updatePlayerHud({
                show,
            ...
            if cache.vehicle and not IsThisModelABicycle(vehicle) then
            ...
            if vehicle and not IsThisModelABicycle(GetEntityModel(vehicle)) then
                if getFuelLevel(vehicle) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        exports.ggcore:Notify(Lang:t("notify.low_fuel"), 'error')
